...
export async function POST(request){
    try {
        ...
        //Check if all requerd fields are present
        ...
        //Check if CouponCode is avelable for new user
        if (couponCode && coupon.forNewUser) {
            const userorders = await prisma.order.findMany({where: {userId}})
            if (userorders.length>0) {
                return NextResponse.json({error: "Coupon valid for only new Users"}, {status: 400})
            }
        }
